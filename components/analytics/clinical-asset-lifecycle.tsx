...
export function ClinicalAssetLifecycle() {
      ...
      <CardContent className="p-10 lg:p-12 space-y-12 flex-1">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 space-y-8">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 italic">{t('clinicalAssetLifecycle.managedDeviceNodes')}</h4>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-3">
                            ...
                            <Badge variant="outline" className={cn("text-[8px] font-black border-white/10 italic", config.color)}>
                              {asset.health}% {t('clinicalAssetLifecycle.integrityScore')}
                            </Badge>
                          </div>
                        </div>
                      </div>
          ...
          <div className="lg:col-span-5 space-y-10">
            <div className="p-8 rounded-[3rem] bg-white/[0.02] border border-white/5 space-y-8 h-full">
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  <div className="p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3">
                    <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest italic">{t('clinicalAssetLifecycle.optimalOperationDelta')}</p>
                    <p className="text-2xl font-black text-white italic tracking-tighter">{t('clinicalAssetLifecycle.yieldValue')}</p>
                    <p className="text-[10px] text-slate-500 font-light italic leading-relaxed">
                      {t('clinicalAssetLifecycle.maintenancePrediction', { node: 'ast2', saving: '฿85,000' })}
                    </p>
                  </div>
                  ...
                  <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5 space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge variant="outline" className="text-[8px] font-black text-blue-400 border-blue-500/20 italic uppercase">{t('clinicalAssetLifecycle.neuralMonitoringOk')}</Badge>
                    </div>
                    <div className="space-y-4">
                      {[
                        t('clinicalAssetLifecycle.tasks.calibration'),
                        t('clinicalAssetLifecycle.tasks.alignment'),
                        t('clinicalAssetLifecycle.tasks.sync')
                      ].map((task, i) => (
              ...
              <Button className="w-full h-16 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest text-[10px] italic shadow-2xl shadow-blue-600/20">
                ...
                {t('clinicalAssetLifecycle.initMaintenance')}
              </Button>
            </div>
          </div>
        </div>
      ...
      <CardFooter className="p-10 lg:p-12 border-t border-white/5 bg-white/[0.01]">
        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4 text-slate-600">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.2em] italic">{t('clinicalAssetLifecycle.lastCalibration', { date: '2026-01-09 04:00 UTC' })}</p>
          </div>
          <p className="text-[9px] font-black text-blue-500/60 uppercase tracking-widest italic">{t('clinicalAssetLifecycle.assetIntegrityLayer')}</p>
        </div>
      </CardFooter>
    </Card>
  )
