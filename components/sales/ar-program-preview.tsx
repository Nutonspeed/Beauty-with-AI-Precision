...
export default function ARProgramPreview({ 
  beforeImage, 
  concerns, 
  recommendations 
}: ProgramPreviewProps) {
  ...
  const generateAfterImage = () => {
    ...
    img.onload = () => {
      ...
      // Get selected program improvement level
      const improvementLevel = recommendations[selectedProgram]?.price > 30000 ? 0.4 : 0.2;
  ...
  }, [beforeImage, selectedProgram, concerns]);
  ...
  const handleDownload = () => {
    ...
    afterImg.onload = () => {
      beforeImg.onload = () => {
        ...
        ctx.fillText(t('arProgramPreview.before'), beforeImg.width / 2 + 20, 40);
        ctx.fillText(t('arProgramPreview.after'), beforeImg.width * 1.5 + 40, 40);
        
        // Download
        const link = document.createElement('a');
        link.download = `program-preview-${Date.now()}.jpg`;
  ...
  return (
    <Card className="w-full">
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('arProgramPreview.title')}
            </CardTitle>
            <CardDescription>
              {t('arProgramPreview.description')}
            </CardDescription>
          </div>
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                alert(t('arProgramPreview.shareComingSoon'));
              }}
            >
      ...
      <CardContent className="space-y-6">
        <Tabs value={selectedProgram.toString()} onValueChange={(v) => setSelectedProgram(parseInt(v))}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm text-muted-foreground">
            <span>{t('arProgramPreview.before')}</span>
            <span>{comparison}%</span>
            <span>{t('arProgramPreview.after')}</span>
          </div>
          <Slider
            value={[comparison]}
        ...
        {/* Improvements List */}
        <div className="space-y-2">
          <h4 className="font-semibold text-sm">{t('arProgramPreview.expectedImprovements')}</h4>
                ...
                <div key={idx} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t('arProgramPreview.severity', { value: concern.severity })}
                    </div>
                  </div>
                  <Badge variant={improvement >= 60 ? 'default' : 'secondary'}>
                    {t('arProgramPreview.improvement', { percent: improvement })}
                  </Badge>
                </div>
              );
            })}
