...
interface ClinicalSentimentProps {
  customerName: string
}

export function ClinicalSentiment({ customerName }: ClinicalSentimentProps) {
  const t = useTranslations()

  const sentimentHistory = [
    { time: t('clinicalSentiment.history.initial'), score: 65 },
    { time: t('clinicalSentiment.history.consult'), score: 72 },
    { time: t('clinicalSentiment.history.scan'), score: 88 },
    { time: t('clinicalSentiment.history.proposal'), score: 94 },
  ]

  const keywords = [
    { word: t('clinicalSentiment.keywords.trust'), weight: 92, color: 'text-emerald-400' },
    { word: t('clinicalSentiment.keywords.precision'), weight: 88, color: 'text-cyan-400' },
    { word: t('clinicalSentiment.keywords.prevention'), weight: 85, color: 'text-pink-400' },
    { word: t('clinicalSentiment.keywords.safety'), weight: 78, color: 'text-amber-400' },
  ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="flex items-center justify-between">
              <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 italic">{t('clinicalSentiment.sentimentTrend')}</h4>
          ...
          <div className="lg:col-span-5 space-y-10">
            <div className="space-y-6">
              <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 italic">{t('clinicalSentiment.keyKeywords')}</h4>
            ...
            <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-pink-600/10 via-transparent to-transparent border border-pink-500/20 space-y-6 relative overflow-hidden group/insight">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <h5 className="text-xs font-black text-white uppercase tracking-widest">{t('clinicalSentiment.aiInsight')}</h5>
              </div>
              <p className="text-[11px] text-slate-400 font-light leading-relaxed italic relative z-10">
                {t('clinicalSentiment.aiInsightDesc')}
              </p>
            </div>
          </div>
        </div>
      ...
      <CardFooter className="p-10 lg:p-12 border-t border-white/5 bg-white/[0.01]">
        <div className="w-full flex items-center justify-between">
          <div className="flex items-center gap-4 text-slate-600">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.2em] italic">{t('clinicalSentiment.bioSentimentSecure')}</p>
          </div>
          <p className="text-[9px] font-black text-slate-700 uppercase tracking-widest italic">{t('clinicalSentiment.engineInfo')}</p>
        </div>
      </CardFooter>
    </Card>
  )
