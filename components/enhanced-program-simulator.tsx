...
import {
  EnhancedProgramSimulator,
  ProgramDefinition,
  CombinedProgramPlan,
} from '@/lib/enhanced-program-simulator';
import {
  Play,
  RotateCcw,
  Plus,
...
interface Props {
  language?: 'th' | 'en';
  onProgramPlanSelect?: (plan: CombinedProgramPlan) => void;
}

const translations = {
  th: {
    title: 'จำลองโปรแกรมความงามขั้นสูง',
    selectPrograms: 'เลือกโปรแกรม',
    availablePrograms: 'โปรแกรมที่มี',
    selectedPrograms: 'โปรแกรมที่เลือก',
    addProgram: 'เพิ่มโปรแกรม',
    removeProgram: 'ลบโปรแกรม',
    simulate: 'จำลอง',
    preview: 'ดูตัวอย่าง',
    stop: 'หยุด',
    reset: 'รีเซ็ต',
    programPlan: 'แผนงานโปรแกรม',
...
export const EnhancedProgramSimulatorComponent: React.FC<Props> = ({
  language = 'en',
  onProgramPlanSelect,
}) => {
  const t = translations[language];
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [availablePrograms, setAvailablePrograms] = useState<ProgramDefinition[]>([]);
  const [selectedProgramIds, setSelectedProgramIds] = useState<string[]>([]);
  const [currentPlan, setCurrentPlan] = useState<CombinedProgramPlan | null>(null);
  const [animationProgress, setAnimationProgress] = useState(0);
  const animationFrameId = useRef<number | undefined>(undefined);

  // Load available programs
  useEffect(() => {
    const programs = EnhancedProgramSimulator.getProgramLibrary();
  ...
  const handleSimulate = () => {
    ...
    try {
      const plan = EnhancedProgramSimulator.createCombinedPlan(selectedProgramIds);
      setCurrentPlan(plan);
      ...
      onProgramPlanSelect?.(plan);
    } catch (error) {
      console.error('Error creating program plan:', error);
    }
  };
  ...
  const renderProgramCard = (program: ProgramDefinition, isSelected: boolean) => (
    <div
      key={program.id}
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div className="flex items-center gap-1">
          ...
          <span>{program.sessionDuration} min</span>
        </div>
        <div className="flex items-center gap-1">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-gray-700">{t.availablePrograms}</h3>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {availablePrograms.map((program) =>
                  renderProgramCard(program, selectedProgramIds.includes(program.id))
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-1">
          {currentPlan ? (
            ...
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 text-center">
              <p className="text-gray-600">{t.noProgramsSelected}</p>
            </div>
          )}
        </div>
      </div>
...
export default EnhancedProgramSimulatorComponent;
