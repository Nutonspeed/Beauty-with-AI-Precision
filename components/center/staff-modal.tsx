  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!editStaff) {
        ...
        payload.center_id = null // Will be set by RLS or app logic
      }

      const url = editStaff 
        ? `/api/center/staff/${editStaff.id}`
        : '/api/center/staff'
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (response.status === 403 && error?.error === 'Sales user limit reached for this center') {
            const maxSalesUsers = error?.details?.maxSalesUsers ?? 1
            toast.error(
              `แพ็กเกจปัจจุบันของเซ็นเตอร์นี้อนุญาตให้มี Sales User ได้สูงสุด ${maxSalesUsers} คน\nหากต้องการเพิ่มเซลมากกว่า ${maxSalesUsers} คน กรุณาอัปเกรดแพ็กเกจ`,
              {
                action: {
                  label: 'ดูแพ็กเกจ',
                  onClick: () => router.push('/center/plans'),
                },
              }
            )
            return
