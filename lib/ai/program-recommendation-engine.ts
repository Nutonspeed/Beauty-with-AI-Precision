 ...
 * AI-powered algorithm to recommend programs based on skin analysis
 */
 ...
 * Analyzes skin concerns and matches with most effective programs
 */
export async function generateProgramRecommendations(
  analysisResults: AnalysisResults,
  criteria: RecommendationCriteria
): Promise<RecommendationResult> {
  // 1. Get all active programs from database
  const allPrograms = await fetchActivePrograms()

  // 2. Score each program based on analysis results
  const scoredPrograms = allPrograms.map(program => ({
    program,
    score: calculateProgramScore(program, analysisResults, criteria)
  }))
  ...
  const recommendations: ProgramRecommendation[] = topPrograms.map((item, index) => ({
    ...
    estimated_cost_min: calculateCost(item.program, criteria, 'min'),
    estimated_cost_max: calculateCost(item.program, criteria, 'max'),
    estimated_sessions: estimateSessions(item.program, analysisResults),
    predicted_improvement: predictImprovement(item.program, analysisResults),
    timeline_weeks: calculateTimeline(item.program),
    recommendation_reason: generateReason(item.program, analysisResults),
    target_concerns: identifyTargetConcerns(item.program, analysisResults),
    status: 'suggested',
...
/**
 * Calculate program score based on effectiveness for detected concerns
 */
function calculateProgramScore(
  ...
  if (analysis.spots && program.effectiveness_spots) {
    const severity = analysis.spots.severity || 0
    const effectiveness = program.effectiveness_spots / 100
    score += severity * effectiveness * 0.6
    factorCount++
  }

  if (analysis.pores && program.effectiveness_pores) {
    const severity = analysis.pores.size || 0
    const effectiveness = program.effectiveness_pores / 100
    score += severity * effectiveness * 0.6
    factorCount++
  }

  if (analysis.wrinkles && program.effectiveness_wrinkles) {
    const severity = analysis.wrinkles.severity || 0
    const effectiveness = program.effectiveness_wrinkles / 100
    score += severity * effectiveness * 0.6
    factorCount++
  }

  if (analysis.texture && program.effectiveness_texture) {
    const severity = 1 - (analysis.texture.score || 0)
    const effectiveness = program.effectiveness_texture / 100
    score += severity * effectiveness * 0.6
    factorCount++
  }

  if (analysis.redness && program.effectiveness_redness) {
    const severity = analysis.redness.level || 0
    const effectiveness = program.effectiveness_redness / 100
    score += severity * effectiveness * 0.6
    factorCount++
  }
  ...
  if (criteria.budget_max) {
    const avgCost = (program.price_min + program.price_max) / 2
  ...
  if (criteria.max_downtime_days !== undefined) {
    if (program.downtime_days <= criteria.max_downtime_days) {
      const downtimeScore = 1 - (program.downtime_days / (criteria.max_downtime_days || 1))
  ...
  if (criteria.max_pain_level !== undefined && program.pain_level !== undefined) {
    if (program.pain_level <= criteria.max_pain_level) {
      const painScore = 1 - (program.pain_level / 10)
  ...
  const popularityScore = program.popularity_score / 100
...
function calculateCost(
  ...
  const basePrice = type === 'min' ? program.price_min : program.price_max
  const sessions = (program.sessions_min + program.sessions_max) / 2
 ...
 */
function estimateSessions(program: Program, analysis: AnalysisResults): number {
  // More severe cases may need more sessions
  const severity = analysis.overall_score ? (1 - analysis.overall_score) : 0.5
  
  if (severity > 0.7) {
    return program.sessions_max
  } else if (severity > 0.4) {
    return Math.ceil((program.sessions_min + program.sessions_max) / 2)
  } else {
    return program.sessions_min
  }
}
 ...
 */
function predictImprovement(program: Program, analysis: AnalysisResults): number {
  // Base improvement on program effectiveness
  const baseImprovement = program.effectiveness_overall
...
/**
 * Calculate program timeline in weeks
 */
function calculateTimeline(program: Program): number {
  const sessions = program.sessions_max
 ...
 */
function generateReason(program: Program, analysis: AnalysisResults): string {
  const concerns: string[] = []

  if (analysis.spots && program.effectiveness_spots && program.effectiveness_spots > 50) {
    concerns.push('dark spots')
  }
  if (analysis.pores && program.effectiveness_pores && program.effectiveness_pores > 50) {
    concerns.push('enlarged pores')
  }
  if (analysis.wrinkles && program.effectiveness_wrinkles && program.effectiveness_wrinkles > 50) {
    concerns.push('wrinkles')
  }
  if (analysis.redness && program.effectiveness_redness && program.effectiveness_redness > 50) {
...
/**
 * Identify which concerns this program targets
 */
function identifyTargetConcerns(program: Program, analysis: AnalysisResults): string[] {
  const targets: string[] = []

  if (analysis.spots && program.effectiveness_spots && program.effectiveness_spots > 40) {
    targets.push('spots')
  }
  if (analysis.pores && program.effectiveness_pores && program.effectiveness_pores > 40) {
    targets.push('pores')
  }
  if (analysis.wrinkles && program.effectiveness_wrinkles && program.effectiveness_wrinkles > 40) {
    targets.push('wrinkles')
  }
  if (analysis.texture && program.effectiveness_texture && program.effectiveness_texture > 40) {
    targets.push('texture')
  }
  if (analysis.redness && program.effectiveness_redness && program.effectiveness_redness > 40) {
...
/**
 * Fetch active programs from database (placeholder)
