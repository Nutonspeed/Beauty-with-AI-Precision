...
`
  ...
  const scriptsDir = path.join(process.cwd(), 'scripts', 'deploy')
  if (!fs.existsSync(scriptsDir)) {
    fs.mkdirSync(scriptsDir, { recursive: true })
  }
  
  fs.writeFileSync(path.join(scriptsDir, 'pre-deploy.sh'), preDeployScript)
  fs.writeFileSync(path.join(scriptsDir, 'post-deploy.sh'), postDeployScript)
  fs.writeFileSync(path.join(scriptsDir, 'rollback.sh'), rollbackScript)
  
  // Make scripts executable
  try {
    exec(`chmod +x ${scriptsDir}/*.sh`)
  } catch (error) {
