...
describe('Role-Based Access Control (RBAC)', () => {
  describe('hasPermission', () => {
    ...
    it('should allow center staff to view center dashboard', () => {
      expect(hasPermission(UserRole.CENTER_STAFF, Permission.VIEW_CENTER_DASHBOARD)).toBe(true)
    })

    it('should NOT allow center staff to manage center settings', () => {
      expect(hasPermission(UserRole.CENTER_STAFF, Permission.MANAGE_CENTER_SETTINGS)).toBe(false)
    })

    it('should allow center admin to manage center settings', () => {
      expect(hasPermission(UserRole.CENTER_ADMIN, Permission.MANAGE_CENTER_SETTINGS)).toBe(true)
    })
  ...
  describe('hasTierFeature', () => {
    ...
    it('should allow aesthetic tier to use aesthetic equivalent', () => {
      expect(hasTierFeature(AnalysisTier.AESTHETIC, 'aesthetic_equivalent')).toBe(true)
    })
  })
  ...
  describe('getRoleLevel', () => {
    it('should return correct hierarchy levels', () => {
      ...
      expect(getRoleLevel(UserRole.CENTER_STAFF)).toBe(3)
      expect(getRoleLevel(UserRole.CENTER_ADMIN)).toBe(4)
      expect(getRoleLevel(UserRole.SUPER_ADMIN)).toBe(5)
    })

    it('should show super admin has highest level', () => {
      expect(isRoleHigherThan(UserRole.SUPER_ADMIN, UserRole.CENTER_ADMIN)).toBe(true)
      expect(isRoleHigherThan(UserRole.CENTER_ADMIN, UserRole.CENTER_STAFF)).toBe(true)
...
describe('Permission Helper Functions', () => {
  describe('canAccessPage', () => {
    ...
    it('should NOT allow free user to access premium pages', () => {
      ...
      expect(canAccessPage('/program-plans', UserRole.FREE_USER)).toBe(false)
    })
    ...
    it('should allow premium customer to access premium pages', () => {
      ...
      expect(canAccessPage('/program-plans', UserRole.PREMIUM_CUSTOMER)).toBe(true)
    })

    it('should NOT allow premium customer to access center pages', () => {
      expect(canAccessPage('/centers/dashboard', UserRole.PREMIUM_CUSTOMER)).toBe(false)
    })

    it('should allow center staff to access center pages', () => {
      expect(canAccessPage('/centers/dashboard', UserRole.CENTER_STAFF)).toBe(true)
      expect(canAccessPage('/centers/customers', UserRole.CENTER_STAFF)).toBe(true)
    })

    it('should NOT allow center staff to access admin pages', () => {
      expect(canAccessPage('/admin/dashboard', UserRole.CENTER_STAFF)).toBe(false)
    })

    it('should allow super admin to access all pages', () => {
      expect(canAccessPage('/admin/dashboard', UserRole.SUPER_ADMIN)).toBe(true)
      expect(canAccessPage('/centers/dashboard', UserRole.SUPER_ADMIN)).toBe(true)
  ...
  describe('canUseFeature', () => {
    ...
    it('should allow center staff to use batch analysis', () => {
      expect(canUseFeature('batch_analysis', UserRole.CENTER_STAFF, AnalysisTier.AESTHETIC)).toBe(true)
    })
  })
  ...
  describe('getDefaultRedirectPath', () => {
    it('should redirect users to correct default pages', () => {
      ...
      expect(getDefaultRedirectPath(UserRole.CENTER_STAFF)).toBe('/centers/dashboard')
      expect(getDefaultRedirectPath(UserRole.CENTER_ADMIN)).toBe('/centers/dashboard')
  ...
  describe('getDefaultTierForRole', () => {
    it('should return correct default tiers', () => {
      ...
      expect(getDefaultTierForRole(UserRole.CENTER_STAFF)).toBe(AnalysisTier.AESTHETIC)
      expect(getDefaultTierForRole(UserRole.CENTER_ADMIN)).toBe(AnalysisTier.AESTHETIC)
    })
  })
  ...
  describe('canUseTier', () => {
    ...
    it('should allow center staff to use aesthetic tier', () => {
      expect(canUseTier(UserRole.CENTER_STAFF, AnalysisTier.AESTHETIC)).toBe(true)
      expect(canUseTier(UserRole.CENTER_ADMIN, AnalysisTier.AESTHETIC)).toBe(true)
    })

    it('should NOT allow free user to use aesthetic tier', () => {
      expect(canUseTier(UserRole.FREE_USER, AnalysisTier.AESTHETIC)).toBe(false)
    })
  })
})
...
describe('Feature Flags System', () => {
  describe('isFeatureEnabledForTier', () => {
    it('should enable browser AI for all tiers', () => {
      ...
      expect(isFeatureEnabledForTier('BROWSER_AI', AnalysisTier.AESTHETIC)).toBe(true)
    })
    ...
    it('should enable AR simulator for premium and aesthetic', () => {
      expect(isFeatureEnabledForTier('AR_SIMULATOR', AnalysisTier.PREMIUM)).toBe(true)
      expect(isFeatureEnabledForTier('AR_SIMULATOR', AnalysisTier.AESTHETIC)).toBe(true)
    })

    it('should enable UV imaging only for aesthetic', () => {
      expect(isFeatureEnabledForTier('UV_IMAGING', AnalysisTier.AESTHETIC)).toBe(true)
  ...
  describe('isFeatureEnabledForRole', () => {
    ...
    it('should restrict UV imaging to center roles', () => {
      expect(isFeatureEnabledForRole('UV_IMAGING', UserRole.CENTER_STAFF)).toBe(true)
  ...
  describe('isFeatureEnabled', () => {
    it('should combine tier and role checks correctly', () => {
      ...
      // Center staff can use UV imaging with aesthetic tier
      expect(isFeatureEnabled('UV_IMAGING', AnalysisTier.AESTHETIC, UserRole.CENTER_STAFF)).toBe(true)
      
      // Premium customer cannot use UV imaging even with aesthetic tier (role restriction)
      expect(isFeatureEnabled('UV_IMAGING', AnalysisTier.AESTHETIC, UserRole.PREMIUM_CUSTOMER)).toBe(false)
    })
  })
  ...
  describe('getEnabledFeaturesForTier', () => {
    it('should return correct number of features for each tier', () => {
      ...
      const aestheticFeatures = getEnabledFeaturesForTier(AnalysisTier.AESTHETIC)
      ...
      // Aesthetic should have most features
      expect(aestheticFeatures.length).toBeGreaterThan(premiumFeatures.length)
    })
  })
...
describe('Security Tests', () => {
  it('should NOT allow privilege escalation', () => {
    ...
    // Center staff should not have admin permissions
    expect(hasPermission(UserRole.CENTER_STAFF, Permission.MANAGE_CENTER_SETTINGS)).toBe(false)
    
    // Sales staff should not access center pages
    expect(canAccessPage('/centers/dashboard', UserRole.SALES_STAFF)).toBe(false)
  })

  it('should properly enforce role hierarchy', () => {
    // Lower roles should not be able to access higher role features
    expect(canAccessPage('/admin/dashboard', UserRole.CENTER_ADMIN)).toBe(false)
    expect(canAccessPage('/centers/dashboard', UserRole.SALES_STAFF)).toBe(false)
    expect(canAccessPage('/ar-simulator', UserRole.FREE_USER)).toBe(false)
  })
