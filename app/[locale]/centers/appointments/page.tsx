...
import { Calendar, Clock, User, Stethoscope, XCircle, CheckCircle2, Loader2, CreditCard, Sparkles } from "lucide-react"
  ...
  useEffect(() => {
    ...
    const loadAppointments = async () => {
      try {
        ...
        const search = `?limit=50${rangeParams}${specialistParam}${statusParam}`
                              ...
                              {a.specialist_id ? (
                                <div className="flex items-center gap-3 group/op">
                                  <div className="h-8 w-8 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center justify-center shrink-0 group-hover/op:bg-blue-500 group-hover/op:text-white transition-all shadow-inner">
                                    <User className="w-4 h-4 text-blue-400 group-hover/op:text-white" />
                                  </div>
