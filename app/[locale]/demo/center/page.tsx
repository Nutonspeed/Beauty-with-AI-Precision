      ...
      <main className="flex-1 relative overflow-hidden flex flex-col">
        ...
        <div className="container relative z-10 py-12 md:py-20 px-6 space-y-16 max-w-7xl mx-auto flex-1">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 pb-12 border-b border-white/5">
            ...
            >
              ...
              <h1 className="text-5xl md:text-8xl font-bold tracking-tighter text-white leading-[0.9] italic">
                {t('demo.center.title')}<br />
                <span className="bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-500 bg-clip-text text-transparent not-italic text-2xl tracking-[0.3em] font-black uppercase">{t('demo.center.subtitle')}</span>
              </h1>
              <p className="text-xl text-slate-500 font-light tracking-widest max-w-2xl italic leading-relaxed">
                {t('demo.center.description')}
              </p>
            </motion.div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {[
              { label: t('demo.center.metrics.inflow'), val: formatCurrency(MOCK_DATA.summary.totalRevenue), icon: DollarSign, trend: MOCK_DATA.summary.growthRate },
              { label: t('demo.center.metrics.cycles'), val: MOCK_DATA.summary.totalBookings, icon: Calendar, trend: 12.4 },
              { label: t('demo.center.metrics.yield'), val: formatCurrency(MOCK_DATA.summary.averageOrderValue), icon: Package, trend: -2.1 },
              { label: t('demo.center.metrics.entities'), val: '1,204', icon: Users, trend: 8.5 }
            ].map((m, i) => (
              <motion.div
                key={i}
            ...
            <div className="lg:col-span-8">
              <Tabs defaultValue="trend" className="space-y-12" value={activeTab} onValueChange={(value: any) => setActiveTab(value)}>
                ...
                <TabsContent value="trend" className="mt-0 outline-none">
                  <Card className="border-white/5 bg-white/[0.01] backdrop-blur-3xl rounded-[3rem] overflow-hidden shadow-2xl relative group">
                    <CardHeader className="p-10 lg:p-12 pb-6 border-b border-white/5">
                      <CardTitle className="text-3xl font-bold text-white tracking-tight italic">{t('demo.center.cards.synthesis')}</CardTitle>
                    </CardHeader>
                ...
                <TabsContent value="payment" className="mt-0 outline-none">
                  <Card className="border-white/5 bg-white/[0.01] backdrop-blur-3xl rounded-[3rem] overflow-hidden shadow-2xl relative group">
                    <CardHeader className="p-10 lg:p-12 pb-6 border-b border-white/5">
                      <CardTitle className="text-3xl font-bold text-white tracking-tight italic">{t('demo.center.cards.vector')}</CardTitle>
                    </CardHeader>
                    <CardContent className="p-10 lg:p-16 h-[400px]">
                      <ResponsiveContainer width="100%" height="100%">
              ...
              <Card className="border-white/5 bg-white/[0.01] backdrop-blur-3xl rounded-[2.5rem] overflow-hidden shadow-2xl relative group p-10">
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-2xl font-bold text-white italic">{t('demo.center.enterprise.title')}</h3>
                    <p className="text-sm text-slate-500 font-light leading-relaxed">
                      {t('demo.center.enterprise.description')}
                    </p>
                  </div>
                  <div className="space-y-4 pt-4">
                    {[
                      t('demo.center.enterprise.features.revenue'),
                      t('demo.center.enterprise.features.inventory'),
                      t('demo.center.enterprise.features.orchestration'),
                      t('demo.center.enterprise.features.lifetime')
                    ].map((feat, i) => (
                  ...
                  <Button variant="premium" className="w-full h-16 rounded-2xl shadow-2xl shadow-pink-500/20 text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95 border" asChild>
                    <Link href={lp("/auth/login")}>{t('demo.center.enterprise.activate')}</Link>
                  </Button>
                </div>
              </Card>

              <div className="rounded-[2.5rem] border border-white/5 bg-white/[0.01] p-10 backdrop-blur-md space-y-6">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 italic text-center">{t('demo.center.diagnostics.title')}</p>
                <div className="space-y-4">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                    <span>{t('demo.center.diagnostics.node')}</span>
