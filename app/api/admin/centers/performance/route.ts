...
export const GET = withAuth(
  async (request: NextRequest, user) => {
    try {
      // Only super_admin can access cross-center data
      ...
      if (centersError) {
        console.error("Admin performance API - centers error:", centersError)
      ...
      // Aggregate by center
      const centerMap = new Map<string, { revenue: number; bookings: number }>()
      let totalRevenue = 0
      let totalBookings = 0
      ...
      // Combine with center info and sort by revenue
      const centerPerformance = (centers || [])
        .map(center => {
          const perf = centerMap.get(center.id) || { revenue: 0, bookings: 0 }
          return {
            id: center.id,
            name: center.name,
      ...
      const result = {
        systemStats: {
          ...
          activeCenters: centers?.length || 0,
        ...
        topCenters: centerPerformance.slice(0, 5), // Top 5 for dashboard
        allCenters: centerPerformance // All 10 for detailed view
      }
